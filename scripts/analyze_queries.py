...
async def main():
    ...
    async with session() as s:
        ...
        print(
            f"{'Query ID':<10} {'Calls':<8} {'Total (s)':<12} {'Mean (ms)':<12} {'Rows':<10} {'Query'}"
        )
        print("-" * 120)
        for row in rows:
            query_preview = (row.query or "?")[:80].replace("\n", " ")
            print(
                f"{row.queryid:<10} {row.calls:<8} {row.total_sec:<12.2f} {row.mean_ms:<12.2f} {row.rows:<10} {query_preview}"
            )
        ...
        for row in rows2:
            print(
                f"{row.schemaname:<15} {row.tablename:<20} {row.indexname:<30} {row.idx_scan:<12} {row.idx_tup_read:<15}"
            )
